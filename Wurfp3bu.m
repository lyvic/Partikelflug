...
## Gekoppelte Bewegungsgleichung lösen	
    ...
    svs=stokesvs(rhop,eta,dp,Vy,x);										%Stokes Speed Vertikal berechnen
    nwvs=[];
	for i = (0:stepset:duration)										%Newton braucht eine for Schleife, da Berechnungen sonst zu Matrixmultiplikationen führen
		nwvs(end+1,1)=newtonvs(rhop,eta,dp,Vy,i);							%Weg-Y-Werte nach Newton errechnen
	endfor
    svp=stokesvp(rhop,eta,dp,Vy,x);
    nwvp=[];
    for i = (0:stepset:duration)
        nwvp(end+1,1)=newtonvp(rhop,eta,dp,Vy,i);
    endfor
    shs=stokeshs(rhop,eta,dp,Vx,x);
    nwhs=[];
    for i = (0:stepset:duration)
        nwhs(end+1,1)=newtonhs(rhop,eta,dp,Vx,i);
    endfor
...
##
	...
	function st = stokeshs (rhop,eta,dp,V_0,x)							
		...
		m=pi()./6.*rhop.*dp.^3;
		st = V_0.*e.^(-3.*1.81E-05.*pi().*dp./m.*x);					
	endfunction
	
	#Newton Wegfunktion horizontal
	function nw = newtonhp (rhop,eta,dp,V_0,x)						
		k=4.*rhop.*dp./3./0.44./1.205;								
		nw =k.*log(((1./k).*V_0.*x)+1);
	endfunction
	
	#Newton Geschwindigkeitsfunktion horizontal
	function nws = newtonhs (rhop,eta,dp,V_0,x)						
		nws =V_0.*(1./((3.*0.44.*1.205.*V_0./4./dp./rhop.*x) +1));
	endfunction
...
##
	...
	#Stokes Wegfunktion
	function st = stokesvp (rhop,eta,dp,V_0,x)								
		beta=18.*eta./rhop./dp.^2;											
		VTS=rhop.*dp.^2.*9.81./18./eta;										
		st = ((-V_0-VTS).*e.^(-beta.*x))./beta - VTS.*x - (-V_0-VTS)./beta;	
	endfunction
	
	#Stokes Geschwindigkeitsfunktion
	function st = stokesvs (rhop,eta,dp,V_0,x)								
		beta=18.*eta./rhop./dp.^2;											
		VTS=rhop.*dp.^2.*9.81./18./eta;										
	...
	#Newton Wegfunktion
	function nw=newtonvp (rhop,eta,dp,V_0,x)
		VTS= sqrt(4.*rhop.*dp.*9.81./3./0.44./1.205);							%VTS nach Newton
		tu= atan(V_0./VTS).*VTS./9.81;										%Umkehrpunkt berechnen
		k=pi()./8.*0.44.*1.205.*dp.^2;										%Konstanten berechnen
		...
		u1=(2./(-V_0./-VTS+1))-1;											%Integrationskonstante berechnen
			if (V_0>0)														%Für Positive Anfangsgeschwindigkeiten
				if (tu>=x)													%Umkehrpunkt mit Zeitpunkt überprüfen
					nw = VTS.^2./9.81.*(log(cos(9.81.*(tu-x)./VTS))-log(cos(9.81.*tu./VTS)));
				endif
				if (x>tu)													%Umkehrzeitpunkt überschritten?
					x=x-tu;													%Umkehrzeitpunkt vom momentanen Zeitpunk abziehen
					nw0 = VTS.^2./9.81.*(-log(cos(9.81.*tu./VTS)));			%Bisher zurückgelegter Weg (Maximale Flughöhe), V_0 steckt in tu.
					nw = nw0-m./k.*log(cosh(sqrt(k.*9.81./m).*x));			%Lösung erstellen
				endif
			endif
		if (0>=V_0)															%Für negative Anfangsgeschwindkeiten
			if (V_0>(-VTS))													%Mit VTS vergleichen, falls VTS noch nicht erreicht
				t=log((2./((V_0./-(sqrt(m.*9.81./k))) +1)) -1)./(-2.*sqrt(k.*9.81./m)); %Zeitpunkt errechnen, der V_0 entspricht
				nw0 = -m./k.*log(cosh(sqrt(k.*9.81./m).*(t)));				%bisher zurückgelegter Weg
				nw = -m./k.*log(cosh(sqrt(k.*9.81./m).*(t+x)))-nw0;			%Momentane Lösung erstellen
			else
	...
	function nw = newtonvs(rhop,eta,dp,V_0,x)								%Funktion für Lösungsvektor nach Newton
		VTS= sqrt(4.*rhop.*dp.*9.81./3./0.44./1.205);							%VTS nach Newton
		tu= atan(V_0./VTS).*VTS./9.81;										%Umkehrpunkt berechnen
		k=pi()./8.*0.44.*1.205.*dp.^2;										%Konstanten berechnen
		m=pi()./6.*rhop.*dp.^3;												%Masse berechnen
	...
	if (V_0>0)																%Für Positive Anfangsgeschwindigkeiten
		if (tu>=x)															%Umkehrpunkt mit Zeitpunkt überprüfen
			nw = VTS.*tan(-9.81.*x./VTS + atan(V_0./VTS));					%Lösung erstellen
		endif
		if (x>tu)															%Umkehrzeitpunkt überschritten?
			x=x-tu;															%Umkehrzeitpunkt vom momentanen Zeitpunk abziehen
			nw = -sqrt(m.*9.81./k).*tanh(sqrt(k.*9.81./m).*x);				%Lösung erstellen
		endif
	endif
	if (0>=V_0)																%Für negative Anfangsgeschwindkeiten
		if (V_0>(-VTS))														%Mit VTS vergleichen,falls VTS noch nicht erreicht
			t=log((2./((V_0./-(sqrt(m.*9.81./k))) +1)) -1)./(-2.*sqrt(k.*9.81./m)); %Zeitpunkt errechnen, der der Anfangsgeschwindigkeit entspricht
			nw = -sqrt(m.*9.81./k).*tanh(sqrt(k.*9.81./m).*(t+x));			%Lösung erstellen
		else
			a=p.*x;															%Exponenten erstellen
...
##
	...
	## Gekoppelte DGLs
		...
		## 
		function 	dy= Vts(x,y,rhop,rhog,eta,dp,windx,windy)
			k=(pi()./8).*rhog.*dp.^2;											%Konstanten berechnen
			c=rhog.*dp./eta;													%Konstanter Teil der Reynoldszahl berechnen
			m=pi()./6.*rhop.*dp.^3;												%Masse des Partikels berechnen
