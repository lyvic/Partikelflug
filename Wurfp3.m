...
## Relaxationszeit abschätzen und Variablen anpassen
    ...
    if (vars(6) == 0)
        ...
        if(duration>60)
            if(Vy>0)
                VTSN= sqrt(4.*rhop.*dp.*9.81./3./0.44./rhog);							%VTS nach Newton
...
## Gekoppelte Bewegungsgleichung lösen	
	...
	for i = (0:stepset:duration)										%Newton braucht eine for Schleife, da Berechnungen sonst zu Matrixmultiplikationen führen
		nwvs(end+1,1)=newtonvs(rhop,rhog,eta,dp,Vy,i);							%Weg-Y-Werte nach Newton errechnen
	endfor
    svp=stokesvp(rhop,eta,dp,Vy,x);
    nwvp=[];
    for i = (0:stepset:duration)
        nwvp(end+1,1)=newtonvp(rhop,rhog,eta,dp,Vy,i);
    endfor
...
##
	...
	#Newton Wegfunktion
	function nw=newtonvp (rhop, rhog,eta,dp,V_0,x)
		VTS= sqrt(4.*rhop.*dp.*9.81./3./0.44./rhog);							%VTS nach Newton
	...
	function nw = newtonvs(rhop,rhog,eta,dp,V_0,x)								%Funktion für Lösungsvektor nach Newton
		VTS= sqrt(4.*rhop.*dp.*9.81./3./0.44./rhog);							%VTS nach Newton
		tu= atan(V_0./VTS).*VTS./9.81;										%Umkehrpunkt berechnen
		k=pi()./8.*0.44.*rhog.*dp.^2;										%Konstanten berechnen
		m=pi()./6.*rhop.*dp.^3;												%Masse berechnen
